...
template = """Bạn là một trợ lý cho các tác vụ hỏi-đáp. Sử dụng các đoạn ngữ cảnh được truy xuất sau đây để trả lời câu hỏi. Không trả lời quá một câu. Đừng dài dòng khi đưa ra câu trả lời. Đưa ra câu trả lời trực tiếp ngay cả khi nó không tạo thành một câu hoàn chỉnh.
Câu hỏi:
* {query} *
...
template_question = """
    [NHIỆM VỤ]: Viết lại câu hỏi dưới đây theo 3 cách khác nhau. Không kèm lời giới thiệu.
    [CÂU HỎI]: {query}
"""
...
# # --- File Processing ---
# input_filename = r"test_data/q.txt"  # Tên file chứa các câu hỏi (mỗi câu 1 dòng)
...
input_filename = r"test_data/questions.txt"  # Questions file (one per line)
output_filename = r"test_data/answers.txt"   # Answers file (one per line)
...
expect_file = r"test_data/expect_answers.txt"
with open(expect_file, 'r', encoding='utf-8') as expect_outfile:
    expect_answers = expect_outfile.readlines()
    expects = [answer.strip() for answer in expect_answers]
